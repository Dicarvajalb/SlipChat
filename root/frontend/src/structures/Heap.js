class Heap{
  ...
  parent(i){
      return Math.trunc((i-1)/2); // 0-Index
  }
  ...
  siftUp(i){ 
      while(i >= 0 && this.heap[i].compareTo(this.heap[this.parent(i)]) == 1 ){
          ...
          let aux = this.heap[this.parent(i)]; //Hace el cambio padre <-> hijo
  ...
  insert(element){
      ...
      if(this.elements == this.size){
        let auxAr = new Array(this.size*2)
      ...
      if(this.elements<this.size){
         this.heap[this.elements] = element;
